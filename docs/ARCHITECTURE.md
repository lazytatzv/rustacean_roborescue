...
│       │   ├── qr_detector_cpp/    # C++: QR コード検出 (WeChatQRCode)
...
#### 4.1 Zenoh P2P 通信
ロボット側とオペレータ側は `rmw_zenoh_cpp` を介して P2P (Peer-to-Peer) で通信する。
- **Mode**: `peer` (ブローカーを介さない直接通信)
- **Transport**: QUIC (Wi-Fi 環境でのパケットロスに強い)

#### 4.2 映像配信 (H.264)
Wi-Fi 帯域を節約しつつ低遅延な映像配信を実現するため、以下のスタックを使用する。
1. **ドライバ**: `v4l2_camera` (Zero-copy コンポーネント)
2. **エンコード**: `ffmpeg_image_transport` (H.264 / libx264)
3. **トピック**: `/camera/image_raw/ffmpeg`
4. **設定**: `preset=ultrafast`, `tune=zerolatency`

#### 4.3 オペレータ GUI
- **Foxglove Bridge**: オペレータ PC 側で起動し、Zenoh 経由で受信したデータを Foxglove Studio へ供給する。
- **デバッグ映像**: JPEG (`compressed`) ではなく H.264 (`ffmpeg`) を優先して使用する。

---

### 3.10 `vision_processor` — QR コードデバッグツール
...
graph LR
  ...
  subgraph NUC[NUC ロボット側]
    ...
    operator -->|/gripper_cmd| arm_driver
    arm_driver -->|/gripper_status| operator
  end
