...
6 自由度アームの手先速度指令を受け取り、Damped Least Squares (DLS) 法による速度逆運動学でジョイント速度に変換するノード。プロフェッショナルレベルの安全機能を実装。
...
| 起動安全 | `/joint_states` 受信前は指令を受け付けない |
...
| ファイル | 目的 |
|----------|------|
| `system.launch.py` | マスター: foxglove_bridge → network → perception → control → nav2 を順に起動 |
