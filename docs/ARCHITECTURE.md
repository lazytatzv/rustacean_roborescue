...
│       │   └── ros2_webrtc/        # Python: WebRTC 音声・映像 (オプション)
...
6DOF アームおよびグリッパの Dynamixel XM サーボに対する統合ドライバ。同一の RS485 バスに接続されたアーム（ID 21-26）とグリッパ（ID 10）を一括管理し、シリアルポートの競合を回避する。
...
`/imu/health`: 直近 2 秒以内にシリアルデータを受信していれば `true`、それ以外 (デバイス未接続・切断中・リトライ中) は `false`。
...
- **IMU 死活監視**: `last_recv` タイムスタンプを共有 Mutex で管理し、1Hz タイマーで健全性を `/imu/health` に配信。`odom_selector` がこれを購読してオドメトリソースを切り替える
...
| `control.launch.py` | ドライバ (crawler, flipper, sensor_gateway, arm_gripper) + コントローラ (joy_controller, arm_controller) |
...
4. **トピック**: `/camera/image_raw/ffmpeg` (Foxglove は H.264 をネイティブデコード)
...
- **映像**: `/camera/image_raw/ffmpeg` (H.264, 2Mbps デフォルト)
...
| `/camera/image_raw/ffmpeg` | CompressedImage (H.264) | v4l2_camera (ffmpeg_image_transport) | foxglove_bridge (operator) |
...
system.launch.py               … NUC 側マスター Launch (foxglove_bridge は含まない)
 ...
 ├─ network.launch.py        … Zenoh ルーター (zenohd --config zenoh_router.json5, QUIC/7447)
 ├─ audio.launch.py          … WebRTC 音声転送 (use_audio=false で無効化可)
 ...
 │   ├─ flipper_driver
 │   ├─ sensor_gateway
 │   ├─ arm_gripper_driver    ← アーム＋グリッパ統合ドライバ
...
| `zenoh_router.json5` | zenohd + ロボット側 ROS 2 ノード | mode=router, listen quic/0.0.0.0:7447, SHM 有効, no multicast |
...
graph LR
  ...
  foxglove_bridge -.->|Zenoh QUIC<br>subscribe all topics| NUC_topics
  ...
  subgraph Camera
    ...
    v4l2 -->|/camera/image_raw/ffmpeg H.264| foxglove_bridge
