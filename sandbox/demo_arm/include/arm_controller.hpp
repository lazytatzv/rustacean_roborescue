...
class ArmCtlNode
{
 private:
  ...
  const int32_t MOTOR_LIMIT[MOTOR_NUM][2] = {
      {-800 * 4, 800 * 4},
      {-900 * 10, 900 * 10},
      {-1500 * 10, 1500 * 10},
      {-4000, 4500},
  ...
  void SetCurrentBasedPositionMode(int motor_index,
                                   bool save_mode);  // motorごとにCurrentBasedPositionModeにセット
  ...
  double value_to_rad(int32_t value, int id) { return value / arm_gear_ratio[id] / 2048; }
