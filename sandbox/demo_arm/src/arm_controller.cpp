...
ArmCtlNode::ArmCtlNode(int looprate) : arm_mock(arm_lengths), arm_solver(arm_lengths)
{
  InitDynamixel();
  SetupSyncHandler();
...
void ArmCtlNode::SetupIndirectAddress(int motor_index)
{
  ...
  result = dxl_wb.writeRegister(dxl_id[motor_index], READ_INDIRECT_MEMORY_FIRST,
                                READ_INDIRECT_MEMORY_SIZE, read_memory_map_array, &log);
  if (result == false)
  {
    ROS_ERROR("%s", log);
...
{
  result = dxl_wb.currentBasedPositionMode(
      dxl_id[motor_index], motor_present_info[motor_index].get_current_limit(), &log);
...
void ArmCtlNode::SetupArmSolver()
{
  ...
  ROS_INFO("[SetupArmSolver] Current Finger Position x: %lf, y: %lf, z: %lf", target_point.x,
...
void ArmCtlNode::MoveHandOpenPos()
{
  ...
  result = dxl_wb.getSyncReadData(present_position_handler_id, &dxl_id[UNDER_ARM_NUM], HAND_NUM,
                                  angles, &log);
  if (!result)
  {
    ROS_ERROR("%s", log);
  ...
  while (!init_position_reached && ros::ok())
  {
    ...
    ROS_INFO("[ARM_CONROLELR] result_vel : %lf, %lf, %lf", result_vel[0], result_vel[1],
             result_vel[2]);
...
void ArmCtlNode::MoveHandResetPos()
{
  ...
  if (angles[0] > wrist_angle[0] && angles[0] < 1920 && (result_vel[1] != 0 || result_vel[2] != 0))
  {
    while (!init_wrist_position && ros::ok())
    {
      ...
      // ROS_INFO("[ARM_CONROLELR] MoveHandResetPos loop1 result_vel : %lf, %lf, %lf",
      // result_vel[0], result_vel[1], result_vel[2]); ROS_INFO("[ARM_CONROLELR] MoveHandResetPos
      // loop1 current_pos : %d, %d, %d", angles[0], angles[1], angles[2]);
  ...
  while (!init_position_reached && ros::ok())
  {
    ...
    // ROS_INFO("[ARM_CONROLELR] MoveHandResetPos loop2 result_vel : %lf, %lf, %lf", result_vel[0],
    // result_vel[1], result_vel[2]); ROS_INFO("[ARM_CONROLELR] MoveHandResetPos loop2 current_pos :
    // %d, %d, %d", angles[0], angles[1], angles[2]);
...
void ArmCtlNode::MoveArmResetPos()
{
  ...
  while (!init_position_reached && ros::ok())
  {
    ...
    result = dxl_wb.syncRead(present_position_handler_id, &log);
    result =
        dxl_wb.getSyncReadData(present_position_handler_id, dxl_id, UNDER_ARM_NUM, angles, &log);
    GetPresentInfomation();  // 関数内のSyncReadを統合したら消す
    SendState();
...
void ArmCtlNode::GetPresentInfomation()
{
  ...
  {
    const int index = i * 2;
    motor_present_info[i].current = (int16_t)memory_data_u32[i] * 2.69;
  }
...
void ArmCtlNode::Reboot_Error_Motor()
{
  for (int i = 0; i < MOTOR_NUM; i++)
  {
    ...
    {
      if (motor_present_info[i].hardware_error != 0b00100000 &&
          motor_present_info[i].hardware_error != 0b00000001)
      {
        ROS_ERROR("Critical Hardware Error Occur id: %d, status: %d", dxl_id[i],
                  motor_present_info[i].hardware_error);
      }
    ...
    if (i == MOTOR_NUM - 1 || i == MOTOR_NUM - 2)
    {  // 指先のモータならそのまま続行
      SetPreviousMode(i);
    }
    else
    {  // それ以外のモータが死んだ場合、復旧時怖いためmode6にしてNormalModeにする
...
Angle4D ArmCtlNode::CalcInversePosition(const geometry_msgs::Vector3 target_addition)
{
  ...
  if (!arm_solver.solve(target_point, 0, angles))
  {  // 1_calc
...
void ArmCtlNode::Callback(const rodep_msgs::Arm_TargetConstPtr &msg)
{
  stored_msg = *msg;
}

void ArmCtlNode::Emergency_Callback(const rodep_msgs::Emergency_State &msg)
{
  Emergency_flag = msg.emergency_stop;
}
...
void ArmCtlNode::StopAllMotor()
{
  ...
  result =
      dxl_wb.syncWrite(goal_velocity_handler_id, dxl_id, ARM_NUM, vel, 1, &log);  // dynamixelに送信
  ...
  switch (mode)
  {
    ...
    case 5:
    {
      ...
      for (int i = 0; i < MOTOR_NUM; i++)
      {
        ...
        {
          ROS_ERROR(
              "[Arm_Controller] Max Rotate Angle!! ID:%d LIMIT:%lf[(abs)rad], Current : %lf[rad]",
              i, value_to_rad(motor_limit[i], i),
              value_to_rad(send_data_value[i] - reset_pos[i], i));
          maxrotflag = true;
        }
      }
      if (maxrotflag)
    ...
    case 6:
    {
      ...
      for (int i = 0; i < MOTOR_NUM; i++)
      {
        if (motor_present_info[i].angle_value < MOTOR_LIMIT[i][0])
        {
          if (send_data_value[i] < 0)
          {
            send_data_value[i] = 0;
            ROS_ERROR(
                "[Arm_Controller] Min Rotate Angle!! ID:%d LIMIT:%lf[(abs)rad], Current : %lf[rad]",
                i, value_to_rad(MOTOR_LIMIT[i][0], i), motor_present_info[i].angle_rad);
          }
        }
        else if (motor_present_info[i].angle_value > MOTOR_LIMIT[i][1])
        {
          if (send_data_value[i] > 0)
          {
            ...
            ROS_ERROR(
                "[Arm_Controller] Max Rotate Angle!! ID:%d LIMIT:%lf[(abs)rad], Current : %lf[rad]",
                i, value_to_rad(MOTOR_LIMIT[i][1], i), motor_present_info[i].angle_rad);
          }
        }
      }
      ...
      dxl_wb.syncWrite(goal_velocity_handler_id, dxl_id, ARM_NUM, send_data_value, 1,
                       &log);  // dynamixelに送信
      dxl_wb.syncWrite(goal_position_handler_id, &dxl_id[ARM_NUM], FINGER_NUM,
                       &send_data_value[ARM_NUM], 1, &log);
