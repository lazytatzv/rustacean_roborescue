...
fn hardware_thread(params: HardwareThreadParams) {
    ...
    'outer: loop {
        ...
        // Port existence check using standard library
...
fn run() -> Result<()> {
    ...
    let arm_ids_raw: Arc<[u8]> = node.declare_parameter("arm_ids").default(Arc::from([1, 2, 3, 4, 5, 6].as_slice())).mandatory()?.get();
    let gripper_ids_raw: Arc<[u8]> = node.declare_parameter("gripper_ids").default(Arc::from([7].as_slice())).mandatory()?.get();
    let arm_dir_raw: Arc<[f64]> = node.declare_parameter("arm_directions").default(Arc::from([1.0; 6].as_slice())).mandatory()?.get();
    let gripper_dir_raw: Arc<[f64]> = node.declare_parameter("gripper_directions").default(Arc::from([1.0; 1].as_slice())).mandatory()?.get();
    ...
    let gripper_off_raw: Arc<[f64]> = node.declare_parameter("gripper_offsets").default(Arc::from([0.0; 1].as_slice())).mandatory()?.get();
    let profile_velocity: i64 = node.declare_parameter("profile_velocity").default(200).mandatory()?.get();
    let gripper_max_current: i64 = node.declare_parameter("gripper_max_current").default(350).mandatory()?.get();
    ...
    let params = HardwareThreadParams {
        ...
        arm_ids: arm_ids_raw.to_vec(),
        gripper_ids: gripper_ids_raw.to_vec(),
    ...
    // Simple status publishing thread to avoid blocking executor
    thread::spawn({
