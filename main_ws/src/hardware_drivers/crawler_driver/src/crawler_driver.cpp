...
class RoboclawDriver
{
 public:
  ...
  bool sendCommand(const std::vector<uint8_t> &data)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("RoboclawDriver"),
                   "Serial read error (timeout or fault): %s",
...
class CrawlerDriver : public rclcpp::Node
{
 ...
 private:
  ...
  void initHardware()
  {
    ...
    if (!roboclaw_.setPIDConstants(M1_SET_PID_CONSTANTS_COMMAND,
                                    static_cast<float>(m1_kp_),
                                    static_cast<float>(m1_ki_),
                                    static_cast<float>(m1_kd_), m1_qpps_))
      RCLCPP_ERROR(get_logger(), "Failed to set PID for M1");
    if (!roboclaw_.setPIDConstants(M2_SET_PID_CONSTANTS_COMMAND,
                                    static_cast<float>(m2_kp_),
                                    static_cast<float>(m2_ki_),
                                    static_cast<float>(m2_kd_), m2_qpps_))
    ...
    RCLCPP_INFO(get_logger(),
                ...
                m1_kp_, m1_ki_, m1_kd_, m1_qpps_,
                m2_kp_, m2_ki_, m2_kd_, m2_qpps_);
  }
