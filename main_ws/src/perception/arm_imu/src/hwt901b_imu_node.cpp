...
HWT901BIMUNode::HWT901BIMUNode() : Node("hwt901b_imu_node"), serial_(io_context_)
{
  ...
  if (baud_rate_ <= 9600)
  {
    RCLCPP_WARN(this->get_logger(), "baud_rate=%d. High-rate outputでは115200推奨です。",
                baud_rate_);
  }

  // 磁気データのみ publish する。
  mag_publisher_ =
      this->create_publisher<sensor_msgs::msg::MagneticField>(kMagneticTopic, kPublisherQueueDepth);
  RCLCPP_INFO(this->get_logger(), "[startup] publisher ready: topic=%s depth=%d", kMagneticTopic,
  ...
  else
  {
    RCLCPP_ERROR(this->get_logger(),
                 "[startup] serial initialization failed; async receive disabled");
  }
...
bool HWT901BIMUNode::configure_serial()
{
  ...
  if (ec)
  {
    RCLCPP_ERROR(this->get_logger(), "Failed to open %s: %s", port_name_.c_str(),
                 ec.message().c_str());
    return false;
  }
...
void HWT901BIMUNode::on_read(const boost::system::error_code &ec, std::size_t n)
{
  ...
  if (ec)
  {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "Serial read error: %s",
                         ec.message().c_str());
    start_async_read();
    return;
  }
  ...
  {
    // 受信データをバッファに追加
    rx_buffer_.insert(rx_buffer_.end(), read_buf_.begin(),
...
void HWT901BIMUNode::decode_frame(const std::array<uint8_t, FRAME_SIZE> &frame)
{
  ...
  msg.magnetic_field.x =
      static_cast<double>(to_int16(frame[2], frame[3])) * GAUSS_TO_TESLA * mag_scale_;
  msg.magnetic_field.y =
      static_cast<double>(to_int16(frame[4], frame[5])) * GAUSS_TO_TESLA * mag_scale_;
  msg.magnetic_field.z =
      static_cast<double>(to_int16(frame[6], frame[7])) * GAUSS_TO_TESLA * mag_scale_;
