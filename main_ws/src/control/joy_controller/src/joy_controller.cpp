...
class JoyController : public rclcpp::Node
{
 ...
 private:
  ...
  ArmInput compute_arm_input(const sensor_msgs::msg::Joy &msg) const
  {
    ...
    // Apply deadzone
    return ArmInput{-apply_deadzone(axis(msg, AXIS_LEFT_Y), deadzone_),
  ...
  void publish_arm(const sensor_msgs::msg::Joy &msg)
  {
    ...
    else if (mode_ == Mode::JOINT)
    {
      ...
      j_msg.name = {"arm_joint1", "arm_joint2", "arm_joint3",
                    "arm_joint4", "arm_joint5", "arm_joint6"};
      j_msg.velocity = {static_cast<double>(input.linear_x * joint_scale_),
